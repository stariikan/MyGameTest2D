...
{
    public static Hero_old Instance { get; set; } //Для сбора и отправки данных из этого скрипта
    ...
    public void Roll()
    {
        ...
        if ((joystickMoveY < -0.4 || vertical < 0) && isGrounded && cooldownTimer > 1.5f && stamina > 15) //кувырок
        {
            ...
            if (!flipRight)
            {
                rb.AddForce(new Vector2(rollForce, rb.velocity.y), ForceMode2D.Impulse);
                cooldownTimer = 0;
            }
            if (flipRight)
            {
                rb.AddForce(new Vector2(rollForce * -1, rb.velocity.y), ForceMode2D.Impulse);
                cooldownTimer = 0;
            }
        }
    }
    public void PlayerMovement()
    {
        ...
        //Run
        //keyboard control
        Vector2 movementX = new Vector2(horizontal, rb.velocity.y); //keyboard
        ...
        //joystick controll
        Vector2 movementJoystickX = new Vector2(joystickMoveX, rb.velocity.y); //tuchpad
