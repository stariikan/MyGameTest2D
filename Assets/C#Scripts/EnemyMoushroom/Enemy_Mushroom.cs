...
public class Enemy_Mushroom : MonoBehaviour //наследование класса сущности (то есть методы которые используются в Entity будут применены и к этому обьекту)
{
    ...
    private float sporesCooldown; //кулдаун атаки спор
    ...
    public void ReboundFromTarget() // отскок от игрока
    {
        ...
        if(playerIsAttack == true)
        {
            ...
            transform.localScale = theScale;
            jumpCooldown = 0;
            float jumpToPlayer = Mathf.Sign(directionX) * 7;
    ...
    public void Attack()
    {
        ...
        if ((Mathf.Abs(directionX)) < 6 && (Mathf.Abs(directionX)) > 2 && jumpCooldown >= 3 && Mathf.Abs(directionY) < 2)
        {
            JumpToPlayer();
        }
        if ((Mathf.Abs(directionX)) < 2 && jumpCooldown > 2 && Mathf.Abs(directionY) < 2)
        {
            ReboundFromTarget();
        }
        if ((Mathf.Abs(directionX)) < 2 && sporesCooldown > 10)
        {
            MushroomSpores();
        }
